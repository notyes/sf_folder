  ...
  $('.menu-collapsible').on('click', 'a', function(e){
    ...
    if($(this).parents('.ps-Menu-list-Left-sub').length > 0){
      ...
      if($(this).is($curParent.find(" > a"))){
        ...
        $(this).siblings('.list-group').children().first().children().click();
        $(this).siblings('.list-group').children().first().children().children().children().first().children().click();
      }else{
        $allSub.removeClass('active');
        $(this).addClass('active');
      }
...
!function($){
    /* *** 
    *
    * There are a problem when tablet display 2 columns.
    *
    // $('.list-item-content').each(function(i,e){
    //     if(i%3==0) $(e).css('clear','left');
    // });
    *
    *  
    *** */
