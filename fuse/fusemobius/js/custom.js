...
!function($){
  ...
  $('.menu-collapsible').on('click', 'a', function(e){
    e.preventDefault();
    if($(this).hasClass('disabled-action')){return false;}
    if($(this).parents('.ps-Menu-list-Left-sub').length > 0){
      ...
      if($(this).hasClass('listmenusub')){
        $allSub.removeClass('active');
        $allParents.removeClass('active');
        $('#listmenusub').addClass('js-menu-expanded');
        $('.list-group-item a:first').addClass('active');
      }
      else
      {
  ...
  if($('a.masterplanhave').size()==1)
  {
    $('a.masterplanhave').click();
  }
  else
  {
